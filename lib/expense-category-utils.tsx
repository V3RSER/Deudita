...
export function getCategoryConfig(category?: string): CategoryConfig {
  if (!category) {
    return { icon: Tag, bgClass: 'bg-emerald-50/90', textClass: 'text-emerald-800', borderClass: 'border-emerald-200/80' };
  }
  const cat = category.toLowerCase();

  if (cat.includes('super') || cat.includes('mercado') || cat.includes('almacen')) {
    return { icon: ShoppingCart, bgClass: 'bg-emerald-100', textClass: 'text-emerald-700', borderClass: 'border-emerald-200' };
  }
  if (cat.includes('abarrote') || cat.includes('despensa')) {
    return { icon: ShoppingBag, bgClass: 'bg-emerald-100', textClass: 'text-emerald-800', borderClass: 'border-emerald-200' };
  }
  ...
  if (cat.includes('hogar') || cat.includes('vivienda') || cat.includes('arriendo') || cat.includes('alquiler')) {
    return { icon: Home, bgClass: 'bg-emerald-100', textClass: 'text-emerald-700', borderClass: 'border-emerald-200' };
  }
  if (cat.includes('restaurante') || cat.includes('comida') || cat.includes('almuerzo') || cat.includes('cena')) {
    return { icon: Utensils, bgClass: 'bg-orange-100', textClass: 'text-orange-700', borderClass: 'border-orange-200' };
  }
  if (cat.includes('delivery') || cat.includes('pedidos') || cat.includes('rappi')) {
    return { icon: PackageCheck, bgClass: 'bg-orange-100', textClass: 'text-orange-800', borderClass: 'border-orange-200' };
  }
  ...
  if (cat.includes('compra') || cat.includes('tienda')) {
    return { icon: ShoppingBag, bgClass: 'bg-indigo-100', textClass: 'text-indigo-700', borderClass: 'border-indigo-200' };
  }
