...
export function FriendsView({ onOpenSettleModal }: Readonly<FriendsViewProps>) {
    const {
        currentProfile,
        profiles,
        members,
                        ...
                        >
                            ...
                            >
                                {friend.avatar_url ? (
                                    ...
                                    <div
                                        className="w-13 h-13 rounded-full bg-zinc-900 text-white flex items-center justify-center text-lg font-bold shrink-0 group-hover:bg-zinc-800 transition-all duration-200">
                                ...
                                <div className="space-y-0.5 overflow-hidden">
                                    <div className="flex items-center space-x-1.5 flex-wrap gap-y-1">
                                        ...
                                        {managedUserIds.includes(friend.id) && (
                                            <span
                                                className="bg-emerald-50 text-emerald-800 border border-emerald-200 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md inline-flex items-center space-x-1">
                                        ...
                                        {sponsorshipMap.has(friend.id) && sponsorshipMap.get(friend.id) !== currentProfile?.id && (
                                            <span
                                                className="bg-zinc-100 text-zinc-700 border border-zinc-200 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md">
                                                Vinculada a {profiles.find(pr => pr.id === sponsorshipMap.get(friend.id))?.full_name?.split(' ')[0] || 'otro'}
                                            </span>
                                        )}
                                        {isTempProfile(friend) && (
                                            <span
                                                className="bg-sky-100 text-sky-800 border border-sky-200 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md">
                            ...
                            {/* Debt Status Card */}
                            <div
                                className="bg-zinc-50 rounded-xl p-4 ring-1 ring-zinc-100 flex items-center justify-between">
                                <span className="text-xs text-zinc-500 font-medium">Estado de Cuentas</span>
                                {debtStatus === 'te_debe' ? (
                                    <span
                                        className="text-sm font-semibold text-emerald-600 flex items-center space-x-1">
                                        <TrendingUp className="w-4 h-4" />
                                        <span>Te debe {formatCurrency(amount)}</span>
                                    </span>
                                ) : debtStatus === 'le_debes' ? (
                                    <span
                                        className="text-sm font-semibold text-rose-600 flex items-center space-x-1">
                                        <TrendingDown className="w-4 h-4" />
                                        <span>Le debes {formatCurrency(amount)}</span>
                                    </span>
                                ) : (
                                    <span
                                        className="text-xs font-semibold text-zinc-600 bg-zinc-200/60 px-2.5 py-1 rounded-md flex items-center space-x-1">
