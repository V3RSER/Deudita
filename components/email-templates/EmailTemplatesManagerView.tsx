                ...
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                            <span
                                className="text-xs font-bold text-zinc-900 truncate">{customTitle || tmpl.name || 'Plantilla'}</span>
                            {entityName && <span
                                className="text-[9px] font-semibold text-zinc-500 bg-zinc-100 px-1.5 py-0.5 rounded shrink-0">{entityName}</span>}
                        </div>
                        <span
                            className="text-[10px] text-zinc-400">{evaluation.criticalFailures.length ? `${evaluation.criticalFailures.length} error(es)` : `${rows.length + 4} comprobaciones`}</span>
                    </div>
                </div>
            ...
            <div className="border-t border-zinc-200/80 p-3 space-y-2.5">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-4 gap-y-1.5">
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-zinc-700 min-w-0">
                        ...
                        <span className="text-zinc-400 shrink-0">→</span>
                        <span
                            className="font-bold text-zinc-800 bg-white border border-zinc-200 px-2 py-0.5 rounded truncate">{evaluation.level4.fields.currency.rawExtracted || '—'}</span>
                    </div>
                </div>
            </div>
        </details>
                      ...
                      {entities.find((e) => e.id === t.entity_id)?.name || 'General'}
                                ...
                                {t.subject_pattern && (
                                    <p className="text-xs text-zinc-500 line-clamp-1">
                                        ...
                                        <code
                                            className="text-[11px] bg-zinc-100 px-1.5 py-0.5 rounded font-mono text-zinc-800">
                                            {t.subject_pattern}
                                        </code>
                                    </p>
                                )}
...
function EmailBodyModal({subject, sender, date, body, onClose}: Readonly<EmailBodyModalProps>) {
    return (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div
                className="bg-white rounded-2xl border border-zinc-200 shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="overflow-y-auto p-5 sm:p-7 select-text cursor-text">
                    <div
                        className="font-mono text-[12px] sm:text-[13px] text-zinc-800 whitespace-pre-wrap leading-7 break-words">{body || '(Sin cuerpo disponible)'}</div>
                </div>
            </div>
        </div>
    );
...
function CustomEmailModal({
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div
                className="bg-white rounded-2xl border border-zinc-200 shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-150">
...
export function EmailTemplatesManagerView({
    ...
    const [expenseTypes, setExpenseTypes] = useState<Array<{
        id: string;
        name: string | null;
        label: string | null
    }>>([]);
    ...
    const handleApplyPastedAIResponse = () => {
        ...
        const resolved = resolveEmailEntity({
            ...
            requestedPattern: data.entity_email_pattern,
            sender,
            body
        });
        ...
        try {
            ...
            if (s) {
                ...
                const resolved = resolveEmailEntity({
                    ...
                    requestedPattern: s.entity_email_pattern,
                    sender: senderForEntity,
                    body: bodyForEntity
                });
                ...
                <div className="bg-white border border-zinc-200 rounded-2xl p-6 sm:p-8 shadow-xs space-y-6 text-center">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-sm text-zinc-600 leading-relaxed max-w-md mx-auto">
                            Conecta tu cuenta de Google para inspeccionar tus correos bancarios y configurar reglas de
                            extracción.
                        </p>
                    </div>
        ...
        <div className="max-w-[1600px] mx-auto space-y-4 px-2 sm:px-0 pb-10">
            <div className="bg-white border border-zinc-200 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="space-y-1 min-w-0">
                        ...
                        <p className="text-xs sm:text-sm text-zinc-500">Revisa correos, identifica la plantilla correcta
                            y crea o corrige reglas de extracción.</p>
                    </div>
            ...
            {activeTab === 'explorer' && (
                <div className="grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-4 items-start">
                    <aside
                        className="bg-white border border-zinc-200 rounded-2xl shadow-xs overflow-hidden lg:h-[calc(100vh-230px)] lg:min-h-[560px] flex flex-col lg:sticky lg:top-4">
                        ...
                        {!selectedEmail ? (
                            ...
                            <div>
                                <div
                                    ...
                                    <div className="min-w-0 flex items-center gap-2.5">
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-600">Editor
                                                de plantilla</p>
                                ...
                                <form onSubmit={handleSaveExplorerTemplate} className="p-4 sm:p-5 space-y-3.5">
                                    ...
                                    <section className="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-3.5">
                                        <div
                                            ...
                                            <div>
                                                <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-600">Asistencia
                                                    IA</p>
                                                <p className="text-xs font-semibold text-zinc-900">Genera una plantilla
                                                    o corrige la actual</p>
                                            </div>
                                    ...
                                    {activeFormEvaluation && (
                                        <section
                                            className={`rounded-2xl border overflow-hidden ${activeFormEvaluation.overallPassed ? 'border-emerald-200 bg-emerald-50/30' : 'border-zinc-200 bg-zinc-50/50'}`}>
                                            <div
                                                className="px-4 py-3 border-b border-zinc-200/70 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                                <div>
                                                    <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">Prueba
                                                        en vivo</p>
                            ...
                            <div className="p-4 sm:p-5 space-y-4">
                                        ...
                                        <div className="flex items-center gap-2.5 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <div
                                                    ...
                                                    <span
                                                        className="truncate">{selectedEmail.sender || '(Sin remitente)'}</span>
                                                    {selectedEmail.date && <span
                                                        className="shrink-0">{new Date(selectedEmail.date).toLocaleString()}</span>}
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <section className="rounded-2xl border border-zinc-200 overflow-hidden">
                                    <div
                                        className="px-4 py-3 bg-white border-b border-zinc-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                    <p className="text-sm font-bold text-zinc-900">Prueba de
                                                        plantillas</p>
                                                    <p className="text-[10px] text-zinc-400">{testReportCounts.matched} coinciden
                                                        · {testReportCounts.failed} no pasan
                                                        · {diagnosisForSelectedEmail?.reports?.length || 0} evaluadas</p>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {testReportCounts.matched > 0 ? (
                                        <div className="p-3 sm:p-4 space-y-3 bg-zinc-50/50">
                                            <div className="flex items-center justify-between gap-2">
                                                <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-700">Plantilla(s)
                                                    que coinciden</p>
                                        ...
                                        <div className="p-4 sm:p-5 bg-zinc-50/60">
                                            <div
                                                className="rounded-2xl border border-dashed border-zinc-300 bg-white p-4 sm:p-5">
                                                <div
                                                    className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                                    <div className="min-w-0">
                                                        <p className="text-xs font-bold text-zinc-900">Este correo no
                                                            tiene una plantilla que lo procese.</p>
                                                        <p className="text-[11px] text-zinc-500 mt-1 max-w-2xl">Desde
                                                            aquí puedes generar la plantilla directamente o copiar el
                                                            prompt para trabajar con la IA. No necesitas ir a otra
                                                            pantalla para comenzar.</p>
                                                    </div>
                                    ...
                                    {testReportCounts.failed > 0 && (
                                        <details className="border-t border-zinc-200 bg-white group">
                                            <summary
                                                className="list-none cursor-pointer px-4 py-3 flex items-center justify-between gap-3 hover:bg-zinc-50 transition [&::-webkit-details-marker]:hidden">
                                                <div>
                                                    <p className="text-xs font-semibold text-zinc-800">Ver evaluaciones
                                                        que no pasaron</p>
                                                    <p className="text-[10px] text-zinc-400">Incluye fallos de entidad,
                                            ...
                                            <div
                                                ...
                                                <div className="space-y-2">
                                                    {filteredTestReports.filter((report) => !(report.level3Passed && report.extractedAmount !== null && report.extractedAmount !== undefined)).length === 0 ? (
                                                        <div
                                                            className="p-5 text-center text-xs text-zinc-500 bg-white rounded-xl border border-dashed border-zinc-200">No
                                                            hay evaluaciones fallidas para este filtro.</div>
                                                    ) : (
