...
export function ProfileSettingsModal({
    ...
    const handleDisconnectGmail = async () => {
        ...
        try {
            ...
            setSuccessMessage('Sincronización de correos desconectada con éxito');
        } catch (err: unknown) {
            setErrorMessage(err instanceof Error ? err.message : 'Error al desconectar');
        } finally {
            ...
            >
                ...
                <form onSubmit={handleSave} className="flex-1 overflow-y-auto px-5 sm:px-6 py-5 space-y-6">
                  ...
                  {currentProfile.email}
                        ...
                        <div
                            className="bg-zinc-50/70 border border-zinc-200/90 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center gap-4 sm:gap-5 shadow-2xs">
                    ...
                    <div className="space-y-3">
                        ...
                        <div
                            className="bg-zinc-50/70 border border-zinc-200/90 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-2xs">
                            {/* Country Selection */}
                      ...
                      Configura tu zona y moneda
                                ...
                                />
                                <p className="text-[11px] text-zinc-500">
                                    Asigna por defecto tu zona horaria y moneda, adaptando los correos y transacciones a
                                    tu horario local.
                                </p>
                            </div>
                  ...
                  opcional
                        ...
                        <div
                            ...
                            <p className="text-xs text-zinc-600 leading-relaxed">
                                Indica los medios por donde prefieres recibir transferencias al saldar cuentas,
                                especificando la plataforma, entidad o enlace y tu respectivo número de cuenta o
                                celular.
                            </p>
                ...
                />
                    ...
                    {/* Section 4: Email Notifications & Banks Sync (Google Apps Script) */}
                    {!isOnboarding && (
                        <div className="space-y-3">
                            <div className="flex items-center justify-between px-0.5">
                                <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-wider flex items-center gap-1.5">
                                ...
                                <span
                                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 ${isLoadingGmailStatus
                  ...
                  {isLoadingGmailStatus ? (
                      ...
                      <span>No conectado</span>
                            ...
                            <div
                                className="bg-gradient-to-br from-zinc-50 via-white to-zinc-50 border border-zinc-200 rounded-2xl p-4 sm:p-5 space-y-3 shadow-2xs">
                                <p className="text-xs text-zinc-600 leading-relaxed">
                                    {gmailConnected
                                        ? 'Tu cuenta está conectada con Google Apps Script para detectar comprobantes de bancos y crear borradores en Tickets automáticamente.'
                                        : 'Conecta tu cuenta de Google mediante Google Apps Script para recibir automáticamente notificaciones bancarias y crear borradores listos para dividir.'}
                                </p>
                                    ...
                                    {gmailConnected ? (
                                        <>
                                            <button
                                                ...
                                                disabled={isConnectingGmail || isDisconnectingGmail}
                                        ...
                                        <>
                                            <button
                                                type="button"
                                                id="profile-connect-gmail-btn"
                                                onClick={handleConnectGmail}
                                                disabled={isConnectingGmail || isLoadingGmailStatus}
                    ...
                    {/* Section 5: Email Templates (Reglas y Formatos Bancarios) */}
                    {!isOnboarding && (
                        <div className="space-y-3">
                            <div className="flex items-center justify-between px-0.5">
                                <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-wider flex items-center gap-1.5">
                            ...
                            <div
                                className="bg-white border border-zinc-200 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-2xs">
                                <div className="space-y-0.5 max-w-md">
                                    <p className="text-xs font-bold text-zinc-900">
                                        Reglas de Extracción y Formatos Bancarios
                                    </p>
                                    <p className="text-xs text-zinc-500 leading-normal">
                                        Configura expresiones regulares, formatos de bancos y prueba la extracción de
                                        montos y comercios directamente con tus correos.
                                    </p>
                                </div>
                                <button
                                    type="button"
                ...
                {/* Footer Actions */}
                <div
                    className="p-4 sm:px-6 border-t border-zinc-100 bg-zinc-50/80 shrink-0 flex items-center justify-end gap-2.5">
                    {!isOnboarding && (
                        <button
