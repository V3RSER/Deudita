 ...
 * Source: lib\email-templates\email-cleaning.ts
...
function createEmailTokenStore() {
    ...
    return {
        protect(text) {
            return text.replace(ANGLE_BRACKET_EMAIL_REGEX, (_match, email) => {
...
function hasSubjectPrefix(subject) {
    ...
    while (true) {
        ...
        if (!SUBJECT_PREFIX_TOKENS.has(token))
            return false;
...
function parseJsonObject(rawText) {
    ...
    try {
        const value = JSON.parse(jsonSubstring);
...
function cleanEmailBody(body) {
    ...
    text = text.replace(WRAPPED_NAMED_HEADER_EMAIL_REGEX, '$1 <$2>');
    text = tokenStore.protect(text);
    if (HTML_DOCUMENT_REGEX.test(text)) {
        text = applyRules(text, HTML_RULES).replace(/&#(\d+);/g, (_match, dec) => String.fromCodePoint(Number.parseInt(dec, 10)));
    }
 ...
 */
function isForwardedEmail(sender, subject, body) {
 ...
 */
function extractInstitutionalSenderEmail(sender, body, isForwarded) {
    const forwardedSender = extractForwardedSenderFromBody(body, DEFAULT_HEAD_LINES);
 ...
 */
function extractForwardedSenderFromBody(body, maxLines = DEFAULT_HEAD_LINES) {
    var _a;
    ...
    if (senderStr.includes('<') && !senderStr.includes('>')) {
        const rest = head.slice(((_a = match.index) !== null && _a !== void 0 ? _a : 0) + match[0].length);
 ...
 */
function inferEntityEmailPattern(sender, body) {
    const forwarded = isForwardedEmail(sender, null, body);
    const institutionalEmail = extractInstitutionalSenderEmail(sender, body, forwarded);
 ...
 */
function extractForwardedSubjectFromBody(body, maxLines = DEFAULT_HEAD_LINES) {
    const head = getHeadLines(body, maxLines);
 ...
 */
function getHeadLines(body, maxLines = 10) {
    if (!body)
        return '';
...
function resolveEmailEntity({ entities, entityId, entityLabel, requestedPattern, sender, body, allowPatternFallback = true, }) {
    ...
    if (entityId) {
        ...
        entity = entities.find((candidate) => candidate.name.trim().toLowerCase() === normalizedLabel) || null;
        if (entity)
            matchedBy = 'name';
    }
    if (!entity && allowPatternFallback) {
        const patternMatch = entities
            .map((candidate) => ({
            candidate,
            match: matchEmailEntityPatterns(candidate.patterns, sender, context.bodyHeadLines, context.forwardedSender),
        }))
    ...
    const senderAlreadyCovered = Boolean(entity &&
        matchEmailEntityPatterns(entity.patterns, sender, context.bodyHeadLines, context.forwardedSender).matched);
...
function buildTemplatePrompt(sender, subject, cleanBody, existingEntities = []) {
    var _a, _b, _c, _d;
    const strippedSubject = stripSubjectPrefixes(subject);
    const forwardedSubject = extractForwardedSubjectFromBody(cleanBody, DEFAULT_HEAD_LINES);
    const effectiveSubject = forwardedSubject ||
    ...
    if (matchedExistingEntity) {
        const existingPatterns = (_b = (_a = matchedExistingEntity.patterns) === null || _a === void 0 ? void 0 : _a.filter(Boolean)) !== null && _b !== void 0 ? _b : [];
        entityInstructions = [
            ...
            ...(existingPatterns.length
                ? [
                    ...
                    ...existingPatterns.map((pattern, index) => `${index + 1}. ${pattern}`),
    ...
    const relevantExistingPatterns = (_d = (_c = matchedExistingEntity === null || matchedExistingEntity === void 0 ? void 0 : matchedExistingEntity.patterns) === null || _c === void 0 ? void 0 : _c.filter(Boolean)) !== null && _d !== void 0 ? _d : [];
    return [
        ...
        ...(relevantExistingPatterns.length
            ? [
                ...
                ...relevantExistingPatterns.map((pattern, index) => `${index + 1}. ${pattern}`),
 ...
 */
function parseAITemplateResponse(rawText) {
    var _a, _b;
    if (!rawText.trim()) {
        return {
            success: false,
    ...
    if (!parsed.name ||
        ...
        const entityLabel = typeof parsed.entity_label === 'string' && parsed.entity_label.trim()
            ? parsed.entity_label.trim()
            : 'Banco';
        parsed.name = `${entityLabel} - Plantilla`;
    }
    if (!parsed.amount_regex ||
    ...
    for (const { key, label, requiresCapture } of EXTRACTION_REGEX_FIELDS) {
        ...
        if (!pattern) {
            const optional = key === 'currency_regex' ||
                key === 'source_account_regex' ||
                key === 'time_regex' ||
                key === 'date_regex' ||
                key === 'merchant_regex';
    ...
    if (parsed.time_regex && !parsed.time_format) {
        validationErrors.push('Si existe time_regex, time_format es obligatorio.');
    }
    ...
    return {
        ...
        data: {
            ...
            amount_regex: normalizeRegexOrNull(parsed.amount_regex) ||
            ...
            date_format: normalizeTemplateString(parsed.date_format, 'DD/MM/YYYY'),
            ...
            source_account_regex: normalizeRegexOrNull(parsed.source_account_regex),
            expense_type: (_b = (_a = normalizeTemplateString(parsed.expense_type)) === null || _a === void 0 ? void 0 : _a.toLowerCase()) !== null && _b !== void 0 ? _b : null,
        },
 ...
 */
function buildCorrectionPrompt(sender, subject, cleanBody, details) {
    var _a, _b;
    const sections = [];
    if ((_a = details.failures) === null || _a === void 0 ? void 0 : _a.length) {
        sections.push('ERRORES BLOQUEANTES QUE IMPIDEN QUE LA PLANTILLA COINCIDA:', ...details.failures.map((failure) => `• ${failure}`));
    }
    const warnings = (_b = details.warnings) !== null && _b !== void 0 ? _b : [];
    if (warnings.length) {
        if (sections.length > 0)
            sections.push('');
        sections.push('AVISOS EN CAMPOS DE EXTRACCIÓN (Revisa los patrones o define null si no aparecen en el correo):', ...warnings.map((warning) => `• ${warning}`));
    }
    if (sections.length === 0) {
    ...
    const forwardedSender = extractForwardedSenderFromBody(cleanBody, DEFAULT_HEAD_LINES);
    const detectedInstitutionalEmail = extractInstitutionalSenderEmail(sender, cleanBody, isForwarded);
    return [
        ...
        '',
        'PLANTILLA ACTUAL:',
        JSON.stringify(pickTemplateFields(details.template), null, 2),
        '',
